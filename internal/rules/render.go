...
// block and would stack/leak. See inject.isEssaimBlock.
const (
	ESSAIM_BEGIN = "<!-- essaim:rules:begin v=1 -->"
	ESSAIM_END   = "<!-- essaim:rules:end -->"
)

// ManagedRegion locates essaim's OWN fenced block in s: the first line-anchored,
// solo-line ESSAIM_BEGIN paired with the next line-anchored, solo-line ESSAIM_END.
...
// unwire can splice from a user's inline sentinel into essaim's block and delete
...
func ManagedRegion(s string) (start, end int, ok bool) {
	for i := 0; ; {
		bi := indexSoloMarker(s, i, ESSAIM_BEGIN)
		if bi < 0 {
			return 0, 0, false
		}
		after := bi + len(ESSAIM_BEGIN)
		endAt := indexSoloMarker(s, after, ESSAIM_END)
		if endAt < 0 {
		...
		nextBegin := indexSoloMarker(s, after, ESSAIM_BEGIN)
		if nextBegin < 0 || endAt < nextBegin {
			return bi, endAt + len(ESSAIM_END), true
		}
...
// matches an essaim-written sentinel but never a marker embedded in a line of
...
// that quoted ESSAIM_END would otherwise create a nested fence the emitter's
...
// learned rule that quotes "<!-- essaim:rules:end -->"). Replacing the inner ':'
