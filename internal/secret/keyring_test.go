...
// (no keyring) skips it. Run locally with: OIKOS_KEYRING_E2E=1 go test ./internal/secret/
func TestKeyringRoundTrip(t *testing.T) {
	if os.Getenv("OIKOS_KEYRING_E2E") == "" {
		t.Skip("needs an OS keyring; set OIKOS_KEYRING_E2E=1 to run")
	}
	k := Keyring{Service: "oikos-test"}
...
func TestEnvOrKeyringPrefersEnv(t *testing.T) {
	e := EnvOrKeyring{
		Keyring: Keyring{Service: "oikos-test"},
		EnvVars: map[string]string{"openrouter-key": "OIKOS_OPENROUTER_KEY"},
		getenv:  func(name string) string { return map[string]string{"OIKOS_OPENROUTER_KEY": "env-secret"}[name] },
	}
...
func TestEnvOrKeyringFallsThroughWhenEnvEmpty(t *testing.T) {
	if os.Getenv("OIKOS_KEYRING_E2E") == "" {
		t.Skip("falls through to the OS keyring; set OIKOS_KEYRING_E2E=1 to run")
	}
	e := EnvOrKeyring{
		Keyring: Keyring{Service: "oikos-test-nonexistent"},
		EnvVars: map[string]string{"openrouter-key": "OIKOS_OPENROUTER_KEY"},
