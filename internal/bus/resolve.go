// Package bus is essaim's opt-in aibus client (Phase 3). It is OFF by default: a
// binary that never runs `essaim join` opens no socket and stays white/local.
// When joined, essaim speaks aibus to the endpoint from the join, scoped to the
...
// Endpoint is where essaim talks to a bus and with which key. Zone is
...
// Resolve returns the bus endpoint essaim should use, applying precedence:
