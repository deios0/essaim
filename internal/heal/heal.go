...
// config file and dropping the oikos proxy base_url — silently bypassing oikos
...
// IDE update DROPS or factory-resets the oikos base_url that oikos itself wrote,
...
// DECISION lives in each Target's Repair seam, which only re-applies oikos's own
...
//
// It is opt-in: `oikos serve` starts it only when tools are wired (there is
// something to keep pointed at the proxy). With no targets it is a no-op. A live
// kill-switch (DisableFlagPath, default <config-dir>/oikos/heal.disabled) turns
// healing off WITHOUT a restart: while the flag file exists, CheckOnce is a
// no-op.
//
// `oikos unwire <tool>` must also take effect WITHOUT a restart (RESPECTS-UNWIRE
...
// Target is one managed config file the watcher keeps pointed at the oikos
// proxy.
//
...
//     "already healthy (holds oikos's managed value)" AND "deliberate user
//     override (holds some other value the user chose)". The watcher writes
//     nothing in either case.
//   - changed == true   ⇒ oikos's OWN previously-written value was clobbered
...
type Target struct {
	...
	// healing a tool that `oikos unwire` removed from the config WITHOUT a restart.
	...
	Path string
	// ExpectedURL is the oikos base_url oikos keeps the file pointed at. It is
	...
	ExpectedURL string
	// LastWritten is the EXACT base_url value oikos itself last wrote into the
	...
	// any OTHER value was changed by something other than oikos. Repair uses it to
	// avoid ever re-applying over a value oikos did not write. May equal
...
// Watcher re-applies the oikos base_url to a set of managed config files whenever
...
type Watcher struct {
	...
	// config surfaced by Repair (P1). It lets `oikos serve` make the failure
...
// user can `touch` the flag to pause, `rm` it to resume; `oikos serve` defaults
// it to <config-dir>/oikos/heal.disabled). An empty path turns the kill-switch
...
// wired-tool names, so a target whose tool was removed by `oikos unwire` on a
...
func (w *Watcher) Start(ctx context.Context) error {
	...
	// Start's caller goroutine, which is `oikos serve`'s setup, not a request). An
...
// to Repair: there is NO "file lacks the oikos url ⇒ rewrite" pre-check (that was
// the ship-blocker — it stomped a base_url a user deliberately set to something
// else). Repair returns changed only when oikos's OWN previously-written value
...
func (w *Watcher) CheckOnce() (int, error) {
	...
	for _, t := range w.targets {
		...
		// wired-tools set by `oikos unwire` is no longer guarded — skip it so the
		...
		// Repair owns the decision: it heals ONLY oikos's own clobbered value and
...
func writeAtomic(path string, content []byte) error {
	...
	tmp, err := os.CreateTemp(dir, ".oikos-heal-*.tmp")
	if err != nil {
		return err
	}
	tmpName := tmp.Name()
