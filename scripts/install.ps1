<#
.SYNOPSIS
    install.ps1 - the `irm https://get.essaim.sh/install.ps1 | iex` installer for essaim (Windows).
...
.DESCRIPTION
    ...
    essaim.exe on a NO-ADMIN PATH dir, and print the next step. It NEVER elevates
    ...
    Offline-testable: set ESSAIM_INSTALL_DRYRUN=1 to print every step (detected
    ...
    Overridable knobs (env):
      ...
      ESSAIM_INSTALL_DIR=<dir>    where to place essaim.exe (default: a no-admin
                                 PATH dir, $env:LOCALAPPDATA\Programs\essaim)
...
function Fail {
    ...
    Write-Error "essaim install error: $Message"
    exit 1
}
...
function Resolve-Arch {
    $override = [Environment]::GetEnvironmentVariable('ESSAIM_ARCH')
...
# $env:LOCALAPPDATA\Programs\essaim (the standard no-admin install location).
...
function Resolve-InstallDir {
    ...
    $override = [Environment]::GetEnvironmentVariable('ESSAIM_INSTALL_DIR')
    if (-not [string]::IsNullOrEmpty($override)) {
        return $override
    }
    ...
    if ([string]::IsNullOrEmpty($localAppData)) {
        if ($IsDryRun) {
            ...
            Fail 'neither LOCALAPPDATA nor APPDATA is set - pass ESSAIM_INSTALL_DIR explicitly'
        }
    }
    return (Join-Win -Parts @($localAppData, 'Programs', 'essaim'))
}
...
function Test-Checksum {
    ...
    if ([string]::IsNullOrEmpty($sums)) {
        Write-Host 'essaim: SHA256SUMS not published for this release; skipping checksum verification'
        return $false
    }
    $want = ''
    ...
    if ([string]::IsNullOrEmpty($want)) {
        Write-Host "essaim: $Asset not listed in SHA256SUMS; skipping checksum verification"
        return $false
    }
    ...
    Write-Host 'essaim: checksum verified'
    return $true
}

# Test-IsFileLocked reports whether Path is a file currently held open by another
# process (e.g. a running essaim.exe). It probes by trying to open the file for
...
# "essaim is running" upgrade case from an ordinary move failure WITHOUT depending
...
function Invoke-Main {
    ...
    $script:BaseUrl = Get-EnvOrDefault -Name 'ESSAIM_BASE_URL' -Default 'https://github.com/deios0/essaim/releases'
    $version = Get-EnvOrDefault -Name 'ESSAIM_VERSION' -Default 'latest'
    $dryRun = (Get-EnvOrDefault -Name 'ESSAIM_INSTALL_DRYRUN' -Default '0') -eq '1'

    # --- detect OS / arch -----------------------------------------------------
    # This script only ever runs on Windows; ESSAIM_OS exists for the test matrix.
    $os = Get-EnvOrDefault -Name 'ESSAIM_OS' -Default 'windows'
    $arch = Resolve-Arch
    ...
    $target = Join-Win -Parts @($binDir, "essaim$ext")

    # --- dry run: plan only, no side effects ---------------------------------
    if ($dryRun) {
        Write-Host 'essaim installer (dry run - no download, no write)'
        Write-Host "  os:        $os"
        ...
        Write-Host '  next step: essaim init   then essaim emit   (writes your AGENTS.md; no proxy needed)'
        Write-Host '  live mode: essaim serve  then open http://127.0.0.1:4141/setup   (optional)'
        return
    }
    ...
    $tmp = Join-Path ([System.IO.Path]::GetTempPath()) ("essaim-" + [System.IO.Path]::GetRandomFileName() + $ext)
    try {
        Write-Host "essaim: downloading $asset ..."
        $proxyParam = Get-ProxyParam
        ...
        # `essaim serve` is running, essaim.exe is locked and Move-Item over it fails
        ...
        # step (stop the running essaim, then re-run) instead of the raw exception.
        #
        ...
        try {
            ...
            if ((Test-Path -LiteralPath $target) -and (Test-IsFileLocked -Path $target)) {
                Fail (
                    "cannot replace $target - it is in use (essaim is running). " +
                    "Stop it first (close ``essaim serve`` / end the essaim.exe process), then re-run this installer."
                )
            }
    ...
    Write-Host "essaim: installed to $target"
    ...
    if ($onUserPath) {
        Write-Host "essaim: $binDir is already on your user PATH"
    } else {
        # Prepend so essaim wins over any older copy; keep the existing User PATH
        ...
        # user types can find essaim without opening a new terminal.
        $env:PATH = "$binDir;$env:PATH"
        Write-Host "essaim: added $binDir to your user PATH (open a new terminal for other apps to see it)"
    }
    Write-Host ''
    Write-Host 'Next:  essaim init     # seed a vault + a starter rule'
    Write-Host '       essaim emit     # write the ranked block into your AGENTS.md (no proxy needed)'
    Write-Host ''
    Write-Host 'Optional live mode (real-time correction capture via the proxy):'
    Write-Host '       essaim serve    # then open http://127.0.0.1:4141/setup'
}
